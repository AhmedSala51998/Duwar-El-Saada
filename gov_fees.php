...
<div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mb-3">
  <h3 class="mb-0">الرسوم الحكومية</h3>
  <div class="d-flex gap-2">
    <form class="d-flex gap-2" method="get">
      <input class="form-control" name="kw" placeholder="بحث بالعنوان" value="<?= esc($kw) ?>">
      <button class="btn btn-outline-secondary">بحث</button>
    </form>
    <a class="btn btn-outline-dark" href="export_gov_fees_excel.php?kw=<?= urlencode($kw) ?>"><i class="bi bi-file-earmark-spreadsheet"></i> Excel</a>
    <a class="btn btn-outline-dark" href="export_gov_fees_pdf.php?kw=<?= urlencode($kw) ?>"><i class="bi bi-filetype-pdf"></i> PDF</a>
    <?php if($can_edit): ?>
      <button class="btn btn-orange" data-bs-toggle="modal" data-bs-target="#addFee"><i class="bi bi-plus-lg"></i> إضافة</button>
    <?php endif; ?>
  </div>
</div>
...
<div class="modal fade" id="edit<?= $r['id'] ?>">
  <div class="modal-dialog"><div class="modal-content">
    <form method="post" action="gov_fee_edit.php" enctype="multipart/form-data">
      <input type="hidden" name="_csrf" value="<?= esc(csrf_token()) ?>">
      <input type="hidden" name="id" value="<?= $r['id'] ?>">
      <div class="modal-header"><h5 class="modal-title">تعديل الرسوم الحكومية</h5><button class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body vstack gap-3">
        <input name="fee_title" class="form-control" value="<?= esc($r['fee_title']) ?>" placeholder="عنوان الرسوم">
        <input name="fee_type" class="form-control" value="<?= esc($r['fee_type']) ?>" placeholder="نوع الرسوم">
        <input type="number" step="0.01" name="fee_amount" class="form-control" value="<?= esc($r['fee_amount']) ?>" placeholder="السعر">
...
<div class="modal fade" id="del<?= $r['id'] ?>">
  <div class="modal-dialog"><div class="modal-content">
    ...
    <div class="modal-body">هل أنت متأكد أنك تريد حذف الرسوم <b><?= esc($r['fee_title']) ?></b> ؟</div>
    <div class="modal-footer">
      <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">إلغاء</button>
      <a href="gov_fee_delete.php?id=<?= $r['id'] ?>" class="btn btn-danger">حذف</a>
    </div>
  </div></div>
</div>
<?php endif; ?>
...
<div class="modal fade" id="addFee">
  <div class="modal-dialog"><div class="modal-content">
    <form method="post" action="gov_fee_add.php" enctype="multipart/form-data">
      <input type="hidden" name="_csrf" value="<?= esc(csrf_token()) ?>">
