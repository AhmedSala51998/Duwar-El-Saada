...
$branchWhereExpenses = '';
$branchParamsExpenses = [];

if ($branch_id !== '' && $branch_id !== 'all') {
    $branchWhereExpenses = " AND branch_id = ? ";
    $branchParamsExpenses[] = $branch_id;
}
...
$sql = "
    ...
    FROM expenses
    WHERE 1 $branchWhereExpenses
    GROUP BY w ORDER BY w DESC
";
$stmt = $pdo->prepare($sql);
$stmt->execute($branchParamsExpenses);
...
// ===============================
// 🟢 Branch Filter للكستوديشس
// ===============================
$branchWhereCustodies = '';
$branchParamsCustodies = [];

if ($branch_id !== '' && $branch_id !== 'all') {
    $branchWhereCustodies = " AND branch_id = ? ";
    $branchParamsCustodies[] = $branch_id;
}
...
$sql = "
    ...
    FROM custodies
    WHERE 1 $branchWhereCustodies
    GROUP BY w ORDER BY w DESC
";
$stmt = $pdo->prepare($sql);
$stmt->execute($branchParamsCustodies);
...
// ===============================
// 🟢 Branch Filter للأصول (Assets)
// ===============================
$branchWhereAssets = '';
$branchParamsAssets = [];

if ($branch_id !== '' && $branch_id !== 'all') {
    $branchWhereAssets = " AND branch_id = ? ";
    $branchParamsAssets[] = $branch_id;
}
...
$sql = "
    SELECT DATE_FORMAT(created_at,'%x-%v') AS w, COUNT(*) AS c
    FROM assets
    WHERE 1 $branchWhereAssets
    GROUP BY w ORDER BY w DESC
";
$stmt = $pdo->prepare($sql);
$stmt->execute($branchParamsAssets);
...
$stmt = $pdo->prepare($sql);
$stmt->execute($branchParamsAssets);
$assetsByWeek_payer_raw = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Assets by Month (Payer)
$sql = "
...
$stmt = $pdo->prepare($sql);
$stmt->execute($branchParamsAssets);
$assetsByMonth_payer_raw = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Assets by Year (Payer)
$sql = "
...
$stmt = $pdo->prepare($sql);
$stmt->execute($branchParamsAssets);
$assetsByYear_payer_raw = $stmt->fetchAll(PDO::FETCH_ASSOC);
